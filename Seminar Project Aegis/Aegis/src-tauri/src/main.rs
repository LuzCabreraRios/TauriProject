...
#[tauri::command]
// NEW: Accept an array of strings from the frontend
fn sanitize_credentials(selected_apps: Vec<String>) -> Vec<String> {
    let mut results = Vec::new();

    if selected_apps.is_empty() {
        results.push("⚠️ No applications selected for sanitization.".to_string());
        return results;
    }
    ...
    // NEW: Filter targets to only include the ones the user clicked on
    let targets: Vec<_> = all_targets
        .into_iter()
        .filter(|(name, _, _)| selected_apps.contains(&name.to_string()))
        .collect();
    ...
    // 2. Second Pass: Attempt deletion
...
// ... Keep the rest of your backend functions exactly as they are below this line ...
#[tauri::command]
